<template>
  <div class="app-container calendar-list-container deploy">
    <div class="filter-container">
      ...
      <el-button class="filter-item pull-right" style="margin-left: 10px;float: right;" @click="deployAll"
                 type="primary" icon="el-icon-news">一键部署
      </el-button>
    </div>
    ...
    >
          ...
          >
            ...
            <el-table-column min-width="130" align="center" :label="$t('table.actions')">
              <template slot-scope="scope">
                ...
                <span
                  v-else-if="scope.row.online === true && (scope.row.ifWait === false || scope.row.ifComplete === true) &&  scope.row.ifRestart === false"
                  @click="deployByComp(scope.row)" style="color: dodgerblue;font-size: 20px;cursor: pointer;">
                  <svg-icon icon-class="deploy"></svg-icon>
                </span>
                <span
                  v-else-if="scope.row.online === true && scope.row.ifComplete === false && scope.row.ifWait === true"
                  style="color: limegreen;font-size: 20px;">
                  <svg-icon icon-class="wait2"></svg-icon>
                </span>
                <span
                  v-else-if="scope.row.online === true && scope.row.ifComplete === true && scope.row.ifRestart === true"
                  @click="deployByComp(scope.row)" style="font-size:20px;color: limegreen;cursor: pointer;">
      ...
      <el-table-column min-width="130" align="center" :label="$t('table.actions')">
        <template slot-scope="scope">
          <el-button size="mini" type="success" :id="scope.row.online" :state="scope.row.state" class="deployBtn"
                     :disabled="!scope.row.online || scope.row.deviceEntity === null"
                     @click="deployByNode(scope.row)" :loading="scope.row.deployLoading">部署
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="部署详情" :visible.sync="dialogTableVisible">
      ...
      <el-table :key='tableKey' :data="deviceDeployDetail" v-loading="listLoading" element-loading-text="给我一点时间" border
